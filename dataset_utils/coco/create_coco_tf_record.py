...
# SETTINGS
allowed_categories = [1]  # What categories to include in the .record
...
def load_coco_dection_dataset(imgs_dir, annotations_filepath, shuffle_img=True):
    ...
    for index, img_id in enumerate(img_ids):
        ...
        for ann in anns:
            # Only record "People"
            if ann['category_id'] not in allowed_categories: continue
        ...
        # print(labels, img_detail['file_name'])
...
def main(_):
    if FLAGS.set == "train":
        ...
        annotations_filepath = os.path.join(FLAGS.data_dir, 'annotations',
                                            'instances_train2014.json')
    ...
    coco_data = load_coco_dection_dataset(imgs_dir, annotations_filepath,
                                          shuffle_img=FLAGS.shuffle_imgs)
